...
Supports North Indian Ocean (local) splits and Global Basin (WP + NA) pre-training datasets.
"""
...
class CycloneForecastingDataset(Dataset):
    ...
    Input X: shape (N, 5, 7) or (N, 5, 3) -> 5 timesteps (t-24, t-18, t-12, t-6, t)
    Target Y: shape (N, 3, 3) -> 3 lead times (+6h, +12h, +24h) [lat, lon, wind_speed]
    """
    def __init__(self, split="train", data_dir="data/processed/forecasting", transform=None):
        ...
        if split == "global_pretrain":
            npz_file = os.path.join(data_dir, "global_pretrain_sequences.npz")
            meta_file = None
        else:
        ...
        self.feature_names = list(data["features"]) if "features" in data else ["lat", "lon", "wind_speed"]
        self.target_names = list(data["targets"]) if "targets" in data else ["lat", "lon", "wind_speed"]
